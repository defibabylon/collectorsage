...
try:
    ...
    app = Flask(__name__)
    CORS(app)
    UPLOAD_FOLDER = 'D:/projects/2024/q3/collectorsage/uploads'
...
def generate_qualitative_report(title, issue_number, year, avg_price, database_avg_price, ebay_data):
    ...
    try:
        ...
        for item in items:
            ...
            location = item.get('itemLocation', {}).get('country', 'Unknown location')
        ...
        best_countries = sorted(location_counts, key=location_counts.get, reverse=True)

        location_tips = "Based on recent listings, the best countries to sell this comic are:\n"
        for country in best_countries:
            location_tips += f"- {country}: {location_counts[country]} listings\n"
        ...
        [short expert justification including interesting geographical insights etc., with citations]
